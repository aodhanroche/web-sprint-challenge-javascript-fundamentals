...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
// The nested function has access to everything in "myFunction" which is where internal is defined.
...
animalNames(zooAnimals);
  ...
  */
  ...
  function lowerCaseNames(data){
    const mappedZooAnimalsLC = data.map((item) => {
      return item.animal_name.toLowerCase();      
    });
    return mappedZooAnimalsLC;
  }
...
console.log(result);
  ...
  */
  ...
  function lowPopulationAnimals(zooAnimals){
    const lowerPop = zooAnimals.filter(animal => animal.population < 5);
    return lowerPop;
  }
  ...
  */
  ...
  function USApop(zooAnimals){
    const sum = zooAnimals.reduce((accumulator, object) => {
